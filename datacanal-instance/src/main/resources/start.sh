...
nohup java ${JAVA_OPTS} -server -classpath "${LIB}/*" ${START_CLASS}  -h ${host} -p ${port} -u ${username} -pw ${password} -n ${dbname} -st ${sensitiveTables} -sp ${positionSyncZkPeriod} -zp ${zkPath} &
