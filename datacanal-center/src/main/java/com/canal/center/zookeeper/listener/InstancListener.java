...
public class InstancListener implements IZkChildListener {
    ...
    public void handleChildChange(String parentPath, List<String> currentChilds) throws Exception {
        ...
        if(null==currentChilds || 0==currentChilds.size()) {
            String dbInfos = (String)zkClient.readData(parentPath);
            //目标数据库的信息
            DbInfo dbInfo = JSON.parseObject(dbInfos, DbInfo.class);
            log.error("instance is lost, need restart in other instance, DBInfo : " + dbInfo.toString());
            
            //TODO haiswang : 到其他机器上启动instance
        } 
    }
}
